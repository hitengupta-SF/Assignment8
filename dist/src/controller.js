...
class controller {
    getAll(req, res) {
        ...
        LEFT JOIN customer ON customer.id = users.customer_id LEFT JOIN role
        ON role.key = users.role_id ORDER BY id ASC`, (error, result) => {
                if (error) {
                    throw error;
                }
                else {
    ...
    getUserById(req, res) {
        ...
        LEFT JOIN customer ON customer.id = users.customer_id LEFT JOIN role
        ON role.key = users.role_idy WHERE users.id = $1`, [id], (error, result) => {
                if (error) {
    ...
    createUser(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            address,customer_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [firstName, middleName, lastName, email, phone, role, address, customer], (err, result) => {
                if (err) {
                    throw err;
                }
                else {
    ...
    updateUser(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
         ...
         phone = $5, role_id = $6, address = $7,customer_id = $8 WHERE id = $9`, [firstName, middleName, lastName, email, phone, role, address, customer, id], (err, result) => {
                if (err) {
